...
export default function Home() {
  ...
  return (
    <div className="min-h-screen relative bg-gradient-to-br from-slate-950 via-blue-950/20 to-indigo-950/30 text-slate-100 overflow-hidden">
      ...
      <div className="absolute top-0 -left-40 w-80 h-80 bg-blue-600/30 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-pulse" 
           style={{ 
             animation: 'blob 7s infinite',
           }}>
      </div>
      <div className="absolute top-0 -right-40 w-80 h-80 bg-purple-600/30 rounded-full mix-blend-multiply filter blur-xl opacity-70" 
           style={{ 
             animation: 'blob 7s infinite 2s',
           }}>
      </div>
      <div className="absolute -bottom-40 left-20 w-80 h-80 bg-pink-600/30 rounded-full mix-blend-multiply filter blur-xl opacity-70" 
           style={{ 
      ...
      {/* Header */}
      <header className={`relative z-10 px-6 py-5 border-b border-white/10 backdrop-blur-md bg-white/5 transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        ...
        <div className="max-w-5xl mx-auto py-16 md:py-24">
          ...
          <div className={`text-center mb-16 transition-all duration-1000 delay-300 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            <p className="text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
              ...
              <span className="text-blue-400 font-medium"> Paste, chat, code.</span>
            </p>
          </div>

          {/* Form section */}
          <div className={`mx-auto max-w-2xl transition-all duration-1000 delay-500 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <div className={`mt-20 grid md:grid-cols-3 gap-8 transition-all duration-1000 delay-700 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            {[
              { icon: '⚡', title: 'Lightning Fast', desc: 'Instant repository indexing and search' },
              { icon: '🧠', title: 'AI Powered', desc: 'Intelligent code analysis and explanations' },
              { icon: '🔒', title: 'Secure', desc: 'Your code stays safe and private' }
            ].map((feature, i) => (
